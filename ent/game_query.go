...
func (gq *GameQuery) QueryAccounts() *GameAccountQuery {
	query := &GameAccountQuery{config: gq.config}
...
func (gq *GameQuery) QueryTransfers() *TransferTransactionQuery {
	query := &TransferTransactionQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withTransfers: gq.withTransfers.Clone(),
...
func (gq *GameQuery) WithAccounts(opts ...func(*GameAccountQuery)) *GameQuery {
	query := &GameAccountQuery{config: gq.config}
...
func (gq *GameQuery) WithTransfers(opts ...func(*TransferTransactionQuery)) *GameQuery {
	query := &TransferTransactionQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withTransfers = query
	return gq
}
...
func (gq *GameQuery) sqlAll(ctx context.Context) ([]*Game, error) {
	...
	if query := gq.withAccounts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Accounts = []*GameAccount{}
		}
		query.withFKs = true
		query.Where(predicate.GameAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(game.AccountsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.game_accounts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "game_accounts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "game_accounts" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Accounts = append(node.Edges.Accounts, n)
		}
	}

	if query := gq.withTransfers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Game)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.TransferTransaction(func(s *sql.Selector) {
			s.Where(sql.InValues(game.TransfersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.game_transfers
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "game_transfers" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "game_transfers" returned %v for node %v`, *fk, n.ID)
			}
