...
func (tx *Tx) init() {
	...
	tx.LineAccount = NewLineAccountClient(tx.config)
	tx.MasterWalletTransaction = NewMasterWalletTransactionClient(tx.config)
	tx.PgSlotAccount = NewPgSlotAccountClient(tx.config)
	tx.PrettyGameAccount = NewPrettyGameAccountClient(tx.config)
	tx.SAGameAccount = NewSAGameAccountClient(tx.config)
	tx.TransferTransaction = NewTransferTransactionClient(tx.config)
