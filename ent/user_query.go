...
type UserQuery struct {
	...
	// eager-loading edges.
	withGames       *GameAccountQuery
	withTransfers   *TransferTransactionQuery
	withBanks       *BankAccountQuery
	withAccessToken *AccessTokenQuery
	withLine        *LineAccountQuery
	withWallet      *MasterWalletTransactionQuery
...
func (uq *UserQuery) QueryGames() *GameAccountQuery {
	query := &GameAccountQuery{config: uq.config}
...
func (uq *UserQuery) QueryTransfers() *TransferTransactionQuery {
	query := &TransferTransactionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryBanks() *BankAccountQuery {
	query := &BankAccountQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryLine() *LineAccountQuery {
	query := &LineAccountQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryWallet() *MasterWalletTransactionQuery {
	query := &MasterWalletTransactionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryChannel() *ChannelQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLine:        uq.withLine.Clone(),
		withWallet:      uq.withWallet.Clone(),
		withChannel:     uq.withChannel.Clone(),
...
func (uq *UserQuery) WithGames(opts ...func(*GameAccountQuery)) *UserQuery {
	query := &GameAccountQuery{config: uq.config}
...
func (uq *UserQuery) WithTransfers(opts ...func(*TransferTransactionQuery)) *UserQuery {
	query := &TransferTransactionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithBanks(opts ...func(*BankAccountQuery)) *UserQuery {
	query := &BankAccountQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithLine(opts ...func(*LineAccountQuery)) *UserQuery {
	query := &LineAccountQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithWallet(opts ...func(*MasterWalletTransactionQuery)) *UserQuery {
	query := &MasterWalletTransactionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithChannel(opts ...func(*ChannelQuery)) *UserQuery {
	query := &ChannelQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withChannel = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withGames; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Games = []*GameAccount{}
		}
		query.withFKs = true
		query.Where(predicate.GameAccount(func(s *sql.Selector) {
		...
		for _, n := range neighbors {
			fk := n.user_games
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_games" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_games" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withTransfers; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Transfers = []*TransferTransaction{}
		}
		query.withFKs = true
		query.Where(predicate.TransferTransaction(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TransfersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_transfers
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_transfers" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_transfers" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Transfers = append(node.Edges.Transfers, n)
		}
	}
	...
	if query := uq.withBanks; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Banks = []*BankAccount{}
		}
		query.withFKs = true
		query.Where(predicate.BankAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(user.BanksColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_banks
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_banks" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_banks" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Banks = append(node.Edges.Banks, n)
		}
	}
	...
	if query := uq.withAccessToken; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AccessToken = []*AccessToken{}
		}
		query.withFKs = true
		query.Where(predicate.AccessToken(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AccessTokenColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_access_token
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_access_token" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_access_token" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.AccessToken = append(node.Edges.AccessToken, n)
		}
	}

	if query := uq.withLine; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Line = []*LineAccount{}
		}
		query.withFKs = true
		query.Where(predicate.LineAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(user.LineColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_line
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_line" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_line" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Line = append(node.Edges.Line, n)
		}
	}
	...
	if query := uq.withWallet; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Wallet = []*MasterWalletTransaction{}
		}
		query.withFKs = true
		query.Where(predicate.MasterWalletTransaction(func(s *sql.Selector) {
			s.Where(sql.InValues(user.WalletColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_wallet
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_wallet" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_wallet" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Wallet = append(node.Edges.Wallet, n)
		}
	}

	if query := uq.withChannel; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if nodes[i].channel_user == nil {
				continue
			}
			fk := *nodes[i].channel_user
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "channel_user" returned %v`, n.ID)
			}
			for i := range nodes {
