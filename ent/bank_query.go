...
func (bq *BankQuery) QueryAccounts() *BankAccountQuery {
	query := &BankAccountQuery{config: bq.config}
...
func (bq *BankQuery) WithAccounts(opts ...func(*BankAccountQuery)) *BankQuery {
	query := &BankAccountQuery{config: bq.config}
...
func (bq *BankQuery) sqlAll(ctx context.Context) ([]*Bank, error) {
	...
	if query := bq.withAccounts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Accounts = []*BankAccount{}
		}
		query.withFKs = true
		query.Where(predicate.BankAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(bank.AccountsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.bank_accounts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "bank_accounts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "bank_accounts" returned %v for node %v`, *fk, n.ID)
			}
